...
void MainWindow::InitThread()
{
    ...
    connect(logger, &qlog::logUpdated, this, &MainWindow::updateLogUI,Qt::QueuedConnection);        // 当日志更新时更新UI
...
void MainWindow::updateUI(float position, float speed, float torque, float current, float errorCode,
...
// 更新电机的实时参数显示
void MainWindow::updateMotorPosition(float position, float speed, float torque, float current, float errorCode,
                                     float position2, float speed2, float torque2, float current2, float errorCode2,
                                     float Acc, quint32 Frame, quint16 Total_status, quint16 Total_error,
                                     float X_position, float X_vec, quint32 X_Status,
                                     float Y_position, float Y_vec, quint32 Y_Status,
                                     ...
                                     quint32 Rob_FB1, quint32 Rob_FB2)
{
    ...
    // 输出每一位，或在需要的地方使用 totalStatus 数组
    //for (int i = 0; i < 16; i++) {
    //    qDebug() << "Bit" << i << ":" << totalStatus[i];
    //}

    // 显示第七位在 textEdit_2 上
    //ui->lineEdit_2->setText(QString::number(totalError[1]));
...
///////////////////////////////////////////////////////远程（读取内存卡数据自运行）/////////////////////////////////////////////////
//        // void MainWindow::readXls() {
//        //     // 使用默认的Excel文件路径
//        //     QString filePath = "C:\\Users\\ASUS\\Desktop\\remote.xls";  // 替换为你的默认文件路径
//        //     if (filePath.isEmpty()) {
//        //         qWarning() << "未指定文件路径";
//        //         return;
//        //     }
...
//        //     excel.setProperty("Visible", false);  // 隐藏 Excel 应用程序窗口
...
//        //             QString cellValue = cell ? cell->property("Value").toString() : "";
//        //             result += cellValue + "\t";  // 使用制表符分隔列
//        //         }
//        //         result += "\n";  // 换行分隔行
//        //     }
...
//        //     // 检查 result 的第一个字符来决定触发哪个按钮的功能
//        //     if (!result.isEmpty()) {
//        //         QChar firstChar = result.at(0);
//        //         if (firstChar == '1') {
//        //             on_btnPower_toggled(true);  // 如果第一个字符是 "1"，触发 btnPowerClicked()
//        //         } else if (firstChar == '2') {
//        //             /*on_btnPower_release();*/  // 如果第一个字符是 "2"，触发其他按钮的功能
//        //         }
//        //     }

//        //     // 将结果显示在 QTextEdit 控件中
...
//        }
        ...
        void MainWindow::clearRfm2gData() {
            ...
            const RFM2G_UINT32 startAddress = 0x4002000;
            const RFM2G_UINT32 endAddress = 0x4002088;
...
void MainWindow::on_readButton_clicked()
{
    QVector<QString> dataTypes = {
            "DWORD", "DWORD", "DWORD",       // 第 1-3 个参数
            "float", "float", "DWORD",      // 第 4-6 个参数
            "float", "float", "DWORD",      // 第 7-9 个参数
            "float", "float", "DWORD",      // 第 10-12 个参数
            "float", "float", "DWORD",      // 第 13-15 个参数
            "float", "float", "DWORD",
            "float", "float", "DWORD",  // 第 16-18 个参数
            "float", "float", "float", "float", "float", "float",
            "float", "float", "float", "float", "float", "float",// 第 19-24 个参数
            "DWORD", "DWORD",              // 第 25-26 个参数
            "float", "float", "float", "float", "float",          // 第 27-31 个参数
            "float", "float", "float", "float", "float",          // 第 32-36 个参数
            "DWORD", "DWORD"                // 第 37-38 个参数
        };
