...
public:
    ...
    // void processMotorData(float position, float speed, float torque, float current, float errorCode,
    ...
    // void onMotorDataProcessed(float position, float speed, float torque, float current, float errorCode,
    //                      float position2, float speed2, float torque2, float current2, float errorCode2,
    //                      const quint16* totalStatus, const quint16* totalError, float Acc, float Frame,
    //                      float X_position, float X_vec, float X_Status, float Y_position, float Y_vec, float Y_Status,
    //                      float Z_position, float Z_vec, float Z_Status, float roll, float roll_vec, float Roll_Status,
    //                      float pitch, float pitch_vec, float Pitch_Status, float yaw, float yaw_vec, float yaw_Status,
    ...
    void updateMotorParameters(float position, float speed, float torque, float current, float errorCode,
                               float position2, float speed2, float torque2, float current2, float errorCode2);
...
QVector<QVariant> readFormattedDataFromMemory (const QVector<QString>& dataTypes);
    ...
    void updateUI(
            float position, float speed, float torque, float current, float errorCode,
                              float position2, float speed2, float torque2, float current2, float errorCode2,
    ...
    void updateMotorPosition(float position, float speed, float torque, float current, float errorCode,
                             float position2, float speed2, float torque2, float current2, float errorCode2,
                             float Acc, quint32 Frame, quint16 Total_status, quint16 Total_error,
                             float X_position, float X_vec, quint32 X_Status,
                             float Y_position, float Y_vec, quint32 Y_Status,
    ...
    /*三个机械的errorCode全局变量*/
    float errorCode_Shux;
    float errorCode_Hengx;
    float errorCode_Robot;
...
///////////////////////////////////////////////////////////////////////PLC控制///////////////////////////////////////////////////////////////////////
    ...
    void on_btnPower_toggled(bool checked);              // 电源按钮状态改变槽函数
                        // 确保UDP连接槽函数
    void resetPowerButtonState(bool checked);            // 重置电源按钮状态槽函数
    void powerOn();                                      // 上电槽函数
    void powerOff();                                     // 下电槽函数
...
signals:
    ...
    void closed();
    void motorDataProcessed(float position, float speed, float torque, float current, float errorCode,
                            float position2, float speed2, float torque2, float current2, float errorCode2,
...
private:
   ...
   // QVector<double> valueData;
    ...
    QWidget *overlay;                   // 遮罩层
    qlog *qlogInstance;                 // qlog实例指针，用于日志记录
