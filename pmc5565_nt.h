 ...
 * with GE Embedded Systems RFM products.
 *------------------------------------------------------------------------
...
typedef struct rfmOpReg_s{
	union {
		...
		ULONG32		ror_ul[RFM_PCIOPREGSIZ / 4];
	}rfmorU;
} rfmOpReg_t, *RFMOR;
...
#define rfm3rr_mode_dma_arbitration		rfmor_rorl[0x08/4]  /*  DMA Arbitration   */
#define rfm2g_bigendian		rfmor_rorc[0x0C]    /* Big/Little Endian config */
#define rfm3rr_misc_control	rfmor_rorl[0x0C/4]  /*  DMA Arbitration   */
...
/*
 *------------------------------------------------------------------------
 * Shape of the rfm3_rev (board revision ) register to identify either the board is classic or PIORC
 *------------------------------------------------------------------------
 */

#define IS_THIS_IS_RFM2G_PIORC_CARD(boardRev) 		(boardRev & 0x80)
#define IS_THIS_IS_RFM2G_CLASSIC_CARD(boardRev) 		((boardRev & 0x80)==0)
...
typedef struct	rfmMapMemory_s	{
	ULONG32			rmm_size;		    /* Total size of the mapped area */
	ULONG32			rmm_freeFireZone;	/* Offset of first user location */
	ULONG_PTR		rmm_rfm;		        /* Mapped address of RFM	     */
 ...
 * one's own node.
 *------------------------------------------------------------------------
 * The IOCTL_INTERNAL_PCI55XX_SEND_INTERRUPT command is for GE Embedded Systems use only.
...
#define	RFM_RSE_BROADCAST	0xFFFF	/* Send to all nodes		 */
	...
	ULONG32		rse_userData;   /* users data to send, ends up in the receive nodes ISD FIFO */
 ...
 * event has occurred. The input argument is a ULONG32; if its value is
...
typedef struct	rfmGetNevents_s	{
	ULONG32		rge_nEvents[RFM2GEVENT_LAST];   /* RFM ireq	*/
	ULONG32		rge_nDma;	    /* Number of DMA operations	    */
	ULONG32		rge_nDmaErr;	/* Number of DMA errors		    */
...
typedef struct rfmEventNotify_s	{
	...
	ULONG32		ren_nEvent;	    /* OUT: Event instance number	 */
	UCHAR		ren_senderId;	/* OUT: RFM Node sending event	 */
	ULONG32		ren_userdata;	/* IN: Userdata from hardware FIFO */
 ...
 * correct operation.  Since these flags are intended for GE Embedded Systems use in
 ...
 * directed by GE Embedded Systems.
 *------------------------------------------------------------------------
 */

typedef struct	rfmDebugFlagName_s	{
	ULONG32		rdfn_bitno;	    /* Binary bit number		    */
	ULONG32		rdfn_mask;	    /* Binary mask value		    */
 ...
 * setting.  Both commands require a ULONG32 paramter.
 *------------------------------------------------------------------------
 */
...
typedef struct rfmGetConfig_s	{
	ULONG32	rgc_registerSize;	/* Bytes consumed by registers	 */
	ULONG32	rgc_memorySize;		/* Amount of RAM memory on board */
	ULONG32	rgc_pciWindowLen;	/* Size of requested PCI window	 */
	ULONG32	rgc_freeFireZone;	/* Beginning of available RAM	 */
	ULONG32	rgc_maxQueueSize;	/* Maximum Event Queue Size      */
	char	rgc_product[32];	/* GE RFM Product Name "SW/RFM2G-WIN" was 64 bytes	*/
...
typedef	struct
{
	...
	ULONG32	DmaRdTot;
	ULONG32	DmaWrTot;
	ULONG32	EvtSndCnt[RFM2GEVENT_LAST];
	ULONG32	EvtRcvCnt[RFM2GEVENT_LAST];
}
DRIVER_INFO;
...
typedef	struct
{
	ULONG32	SlidingWindowOffset;
	ULONG32	SlidingWindowOffsetEnable;
} SLIDING_WINDOW_SET;
#endif
